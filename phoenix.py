...
#       """get the foam xoffset"""
    ...
    def construct(self,foam_param):
        ...
        irow = irow+2
        canvas_for_image = Canvas(tab3, bg='white', height=500, width=600, borderwidth=10, highlightthickness=0)
...
def FoamBlock_default_set_up():
    """ default machine  set up:
        ...
        - speed ...   """
    foam_param=[]
    foam_param.append("50.0")  # x offset (mm)   [0]
    foam_param.append("10.0")   # y offset (mm)   [1] 
    foam_param.append("0.0")  # z offset (mm)   [2] 
    foam_param.append("10.0")  # overlength leading edge (mm)    [3]     
    foam_param.append("10.0")  # overlength trailing edge (mm)   [4]        
    foam_param.append("40.0")  # foam block thickness (mm)       [5]       
    return foam_param
...
#       """get the kerf """
    ...
    def construct(self,hotwire_param):
        ...
        # Image Machine  https://www.diyrcwings.com/app/
        irow = irow+1
        canvas_for_image = Canvas(tab2, bg='white', height=500, width=600, borderwidth=0, highlightthickness=0)
...
class Panel :
    ...
    def __init__(self,n):
        ...
        # Will be charge at export in calcgc
        self.rootnpts = 0
        self.tipnpts  = 0
        
        # before  kerf correction  
        self.root_X =  []
        self.root_Y_ext0 =  []
        self.root_Y_int0 =  []
        self.root_Z_ext0 =  []
...
#         """get the tip chord length,  set the root chord length for the nex panel"""
    ...
    def select_file_afr(self):
         ...
         dirl= f.split("/")
         self.lblafr = Label(self.frame, text= " ", bg="white")
    ...
    def select_file_aft(self):
         ...
         if npanel > 1 :
              if ((self.Panel_Number+1) < npanel ) :
                  ...
                  dirl= f.split("/")
                  p.lblafr= Label(p.frame, text= " ", bg="white")
...
def load_default_wing():
    ...
    for ip, p in enumerate(panel_list):
        ...
        dirl= f.split("/")
        p.lblaft = Label(p.frame, text= " ", bg="white")
...
def fct_draw_wing():
    ...
    if status == False :
        ...
        for p in panel_list:
            ...
            tanalfa = math.tan( float(p.sweepangle)*d2r)
            ...
            p2y = p1y + float(p.panel_span)*tanalfa 
        ...
        scaley = wing_canvas_height / (ymax+abs(ymin) +2*ymargin ) 
        ...
        for ip, p in enumerate(panel_list): 
            ...
            p2y = p1y + float(p.panel_span)*tanalfa*scale
            ...
            dyle = float(p.panel_span)*tanalfa
            if (abs(dyle) > 0) :
                wing_canvas.create_text ((p2x+10), 0.5*(p1y+p2y), text= "Dl LE "+str("{:.1f}".format(dyle))+str("mm"),
                              fill= list_color[ip], font= ("courier", 8, "bold italic"), justify= "right",angle=90)  
                wing_canvas.create_line(  (p2x, p1y), (p2x, p2y) , dash=(1,1),
                              fill=list_color[ip], width=1, smooth=False, arrow=tk.BOTH)   
                wing_canvas.create_line(  (p1x, p1y), (p2x, p1y) , dash=(1,1),
                              fill=list_color[ip], width=1, smooth=False)    
...
def save_model():
    ...
    for ip, p in enumerate(panel_list):
        ...
        save_model_file.write("0.0".strip('\n')+'\n') 
...
if __name__ == '__main__':
    ...
    root.iconbitmap('./IMAGE/phoenix.ico')
    ...
    irow = irow+1
    canvas_for_image = Canvas(tab4, bg='white', height=500, width=600, borderwidth=0, highlightthickness=0)
